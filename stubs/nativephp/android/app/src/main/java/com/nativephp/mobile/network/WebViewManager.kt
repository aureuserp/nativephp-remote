...
class WebViewManager(
                        ...
                        if (path != null && path.startsWith("/_native/")) {
                            ...
                            val postData = if (request.method.equals("POST", ignoreCase = true) ||
                                ...
                                if (reqId != null) {
                                    ...
                                    // Fallback: try original URL, then path
                                    phpBridge.consumePostData(url)
                                        ?: phpBridge.consumePostData(path)
                                }
                            } else null
    ...
    private fun injectJavaScript(view: WebView) {
        val hostedRemoteMode = LaravelEnvironment.getHostedRemoteHost(context) != null

        val jsCode = if (hostedRemoteMode) {
            """
            (function() {
                const listeners = {};
                ...
                return "Hosted remote native bridge installed";
            })();
            """.trimIndent()
        } else {
            """
